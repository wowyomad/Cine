	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					CINE_CORE_ERROR("Instrumentor::BeginSession('{}') when session '{}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			else
			{
				if (Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
					CINE_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
...
#define CINE_FUNC_SIG __PRETTY_FUNCTION__
#elif defined(__DMC__) && (__DMC__ >= 0x810)
#define CINE_FUNC_SIG __PRETTY_FUNCTION__
#elif (defined(__FUNCSIG__) || (_MSC_VER))
#define CINE_FUNC_SIG __FUNCSIG__
#elif (defined(__INTEL_COMPILER) && (__INTEL_COMPILER >= 600)) || (defined(__IBMCPP__) && (__IBMCPP__ >= 500))
#define CINE_FUNC_SIG __FUNCTION__
#elif defined(__BORLANDC__) && (__BORLANDC__ >= 0x550)
#define CINE_FUNC_SIG __FUNC__
#elif defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901)
#define CINE_FUNC_SIG __func__
#elif defined(__cplusplus) && (__cplusplus >= 201103)
#define CINE_FUNC_SIG __func__
#else
#define CINE_FUNC_SIG "CINE_FUNC_SIG unknown!"
#endif	
...
#else
#define CINE_PROFILE_BEGIN_SESSION(name, filepath)
#define CINE_PROFILE_END_SESSION()
#define CINE_PROFILE_SCOPE(name)
#define CINE_PROFILE_FUNCTION()
#endif