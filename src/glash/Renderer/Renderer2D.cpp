...
namespace Cine
{
	...
	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const Ref<SubTexture2D>& subtexture, float tiling, const glm::vec4& tint)
	{
		glm::mat4 transform = glm::scale(glm::translate(glm::mat4(1.0f), position), glm::vec3(size.x, size.y, 1.0f));
	...
	void Renderer2D::DrawQuad(const glm::mat4& transform, const Ref<SubTexture2D>& subtexture, float tiling, const glm::vec4& tintColor)
	{
		CINE_PROFILE_FUNCTION();
		...
		if (s_Data.QuadIndexCount == s_Data.MaxIndices)
		{
			...
			s_Data.QuadIndexCount = 0;
			s_Data.QuadVertexBufferPtr = s_Data.QuadVertexBufferBase;
			DrawQuad(transform, subtexture, tiling, tintColor);
		}
		constexpr size_t quadVertexCount = 4;
